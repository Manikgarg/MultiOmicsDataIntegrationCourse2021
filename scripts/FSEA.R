...
runEnrichmentAnalysisUsingSeparateWeightsAndFactors <- function(weightList, factorList, data, view,
  ...
  if (paste0(factors,collapse="") == "all") { 
    ...
    factors <- colnames(Z)[factors]
  } else { 
    stopifnot(all(factors %in% colnames(Z))) 
  }
...
plotEnrichmentUsingSeparateWeightsAndFactors <- function(fsea.results, methodName, factor, alpha=0.1, max.pathways=25, adjust=TRUE) {
  ...
  #if(is.numeric(factor)) factor <- colnames(Z)[factor]
